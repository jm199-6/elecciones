<br><br><br>
<?php
	$sql = "SELECT u.dui, u.nombres, u.apellidos, u.direccion, c.pass, p.nombre_priv, p.id_priv from usuario u inner join cuenta c on c.idUsuario=u.idUsuario inner join privilegio p on c.idPriv = p.id_priv where dui = '".$_SESSION["ID"]."'";
	...
	<div class="panel panel-primary">
		...
		<div class="panel-body">
			<form method="post">
				...
				<input type="text" class="form form-control" id="direccion" name="direccion" value="<?php echo $row['direccion'] ?>" onKeyUp="this.value=this.value.toUpperCase();" disabled>
				<br>
				<label >CONTRASEÑA</label>
				<input type="password" class="form form-control" id="p" name="p" value="<?php echo desencript($row['pass']); ?>"  disabled>
...
<?php
	...
	if(isset($_POST['ok'])){
		...
		$sql = "UPDATE usuario set nombres='".$nombres."', apellidos='".$apellidos."', direccion='".$direccion."' WHERE dui='".$dui."'";
		$sqlCuenta = "UPDATE cuenta set pass='".$p."', idPriv='".$id_priv."' where idUsuario='".$dui."'";
		if($conn->query($sql) == true){
			$_SESSION["nombre"]=$nombres." ".$apellidos;
			if($conn->query($sqlCuenta) == true){

				echo "<script>alert('Perfil actualizado satisfactoriamente.');
				document.location='./?a=".encript("profile.php")."';
				</script>";
			}
		}
	}
